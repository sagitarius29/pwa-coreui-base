<template>
  <b-card :header="headerTitle">
    ...
    <div>
      ...
      <form-import-component :endpoint="endpoint" ref="formImport"/>
    </div>
  </b-card>
</template>
...
<script>
  ...
  export default {
    ...
    components: {FormImportComponent, DataCrudComponent, DataTableComponent},
    props: {
      headerTitle: {
        default: ''
      },
    ...
    methods: {
      loadTableFields() {
        return custom_table.getColumns(this.customTable).then(({data}) => {
          this.loadedTableFields = _.map(data, (item) => {
            console.log('item.type')
            if(item.type === 'tags') {
              item.callback = this.callbackTags
            }
            return item;
          });
          this.loadedTableFields.push({
            title: 'Acciones',
            name: '__slot:actions'
          });
        });
