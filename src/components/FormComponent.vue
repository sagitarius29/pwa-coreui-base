  ...
  <div class="row">
    <div :class="input.class || 'col-lg-12'" v-for="input in schema">
      <div class="form-group">
        ...
        <dynamic-select v-else-if="input.type === 'dynamic-select'" v-model="document[input.bind]" :config="input.config"/>
        <static-select v-else-if="input.type === 'static-select'" v-model="document[input.bind]" :config="input.config"/>
        <date-input v-else-if="input.type === 'date'" v-model="document[input.bind]"/>
...
<script>
  ...
  import StaticSelect from "./inputs/StaticSelect";
  import TagInput from "./inputs/TagsInput";
  export default {
    name: "FormComponent",
    components: {StaticSelect, DateInput, DynamicSelect, TagInput},
    props: {
      schema: {
        default() {
          return {};
